...
// import { useParams, useNavigate } from 'react-router-dom';
// import axios from 'axios';
// import { Card, Col, Row, Button, Select, InputNumber, Typography, Spin, Alert } from 'antd';
// import Navbar from '../../components/Navbar';
// import { Container } from 'react-bootstrap';
...
//         // If your backend returns relative image path, build full URL:
//         // Adjust this if your API returns full URLs already
//         if (productData.image && !productData.image.startsWith('http')) {
//           productData.image = `http://localhost:3001/product/${productData.image}`;
//         }
...
//               preview={{ mask: <div>Click to zoom</div> }}
...
const ProductDetailsPage = () => {
  const { id } = useParams(); // This should be the numeric product ID like 1000
  const [product, setProduct] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/api/products/${id}`)
      .then(response => {
        ...
        // Construct full image URL only if it's not already a full path
        if (productData.image && !productData.image.startsWith('http')) {
          productData.image = `http://localhost:5000/uploads/${productData.image}`;
        }
      ...
      <div className="container mt-4">
        <div className="row">
          ...
          <div className="col-md-6">
            ...
            <p><strong>Stock:</strong> {product.stock ?? 'N/A'} {product.measurement_unit}</p>
            <Button type="primary" className="me-2">Add to Cart</Button>
            <Button type="default">Buy Now</Button>
          </div>
        </div>
