...
exports.addPurchase = async (req, res) => {
  const { product_id, productName, dealer_id, dealer_name, quantity, purchase_price } = req.body;

  if (!product_id || !productName || !dealer_id || !dealer_name || !quantity || !purchase_price) {
  ...
  try {
    ...
    await db.query(
      `INSERT INTO purchases (product_id, productName, dealer_id, dealer_name, quantity, purchase_price)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [product_id, productName, dealer_id, dealer_name, quantity, purchase_price]
    );
